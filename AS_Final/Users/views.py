...
# 		SUB-WORKOUT LOOP # 1
		for i in _Workout.SubWorkouts.all():
			...
			if Template.Strength_Stop > 0:
				StrengthStop = True
				Row["Pre_SR"] = ["Strength Stop: " + str(i.Template.Strength_Stop)]
				Row["Sets_Reps"] =  str(i.Template.Reps) + " @ " + str(i.Template.RPE) + " RPE"
				Row["Additional_Info"] = "Strength Stop"
			...
			if (i.Exercise.Video != None):
				Row["Has_Video"] = True
				Row["Video_PK"] = i.Exercise.Video.pk
...
#				ADDING UNFILLED SETS AS ROWS 
			...
			if i.Template.Stop_Set or i.Template.Drop_Set or i.Template.Strength_Drop > 0:
				Empty_Sets = 1
